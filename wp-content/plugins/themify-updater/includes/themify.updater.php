...
if ( !class_exists('Themify_Updater') ) :
    ...
    class Themify_Updater
    {
        ...
        private static $files_included = false;
        ...
        */
        ...
        public function themify_reinstall_theme ($theme) {
            ...
            $back_version = $this->version->has_attribute($theme, 'skip_version_start', true);
			$version = $this->version->remote_version($theme);
			?>
            <p class="update">
                <select id="themeversiontoreinstall" name="version">
					<option selected="selected" value=""><?php _e( 'Latest version', 'themify-updater' ); ?></option>
					<?php
						if ( !empty($back_version) ) {
						    $version_end = Themify_Updater_utils::next_version( $this->version->has_attribute($theme, 'skip_version_end', true) );
							$versions = Themify_Updater_utils::get_previous_versions( $version, 5, false, $version_end);
							if ( count($versions) < 5) {
								$versions = array_merge($versions, Themify_Updater_utils::get_previous_versions($back_version, 5 - count($versions), false, '0.0.1', true) );
							}
							//Get last stable version of theme.
							...
							foreach ( $versions as $v ) {
								...
								if (!empty($lsv) && $lsv === $v) {
									...
									echo '<option value="'. $v .'" title="'. __('Last Most Stable Theme Version', 'themify-updater') .'">'. $v .'</option>';
								} else {
							...
							if (!$has_stable_version && !empty($lsv)) {
								echo '<option value="'. $lsv .'" title="'. __('Last Most Stable Theme Version', 'themify-updater') .'">'. $lsv .'</option>';
							}
						} else {
							echo Themify_Updater_utils::get_previous_versions( $version, 5, true );
						}
					?>
				</select>
            </p>
        ...
        function older_theme_updater($response, $url) {
			...
			if ( 0 === strpos( $url, 'https://api.wordpress.org/themes/update-check' ) ) {
				if ( function_exists('themify_hide_themes') && !current_theme_supports( 'themify-exclude-theme-from-wp-update' ) ) {
