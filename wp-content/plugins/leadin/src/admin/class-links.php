...
class Links {
	...
	/**
	 * Get routes for the oauth version of the app
	 */
	public static function get_oauth_routes_mapping() {
		$portal_id = Connection::get_portal_id();
		return array(
			MenuConstants::ROOT     => "/$portal_id/onboarding/start",
			MenuConstants::SETTINGS => "/$portal_id/settings",
		);
	}
	 ...
	 */
	public static function get_query_param_array() {
		$query_params = array(
			'l'        => get_locale(),
	...
	public static function get_query_params() {
		$query_param_array = self::get_query_param_array();
	...
	/**
	 * From the page id, return a corresponding route to the iframe.
	 */
	private static function get_path_from_page_id() {
		$page_id = self::get_page_id();
		$routes  = self::get_oauth_routes_mapping();
	...
	public static function get_iframe_src() {
		$path         = '/onboarding/';
		$query_params = self::get_query_param_array();
		if ( OAuth::is_enabled() ) {
			$query_params['oauth'] = OAuth::is_enabled();
			if ( Connection::is_connected() ) {
				$path = self::get_path_from_page_id();

				if ( isset( $_GET['justConnected'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$query_params['justConnected'] = true;
				}
			}

			return LeadinFilters::get_leadin_base_url() . '/wordpress-plugin-ui' . "$path" . '?' . http_build_query( $query_params );
		} else {
